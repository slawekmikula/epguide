...
class Config(object):
    ...
    def __init__(self):
        ...
        self.cmdparser.add_option ("-l", dest="list", action="store_true", default=False, help="list all channels and their numbers for a given provider")        
        self.cmdparser.add_option ("-c", dest="channel_list",  help="input channel list (index from -l command) comma separated or list with format parser|channel comma separated")
        self.cmdparser.add_option ("-d", type="date", dest="date", help="date you want the program from (format: [YYYY-MM-DD])")
        self.cmdparser.add_option ("-f", dest="output", choices=["txt","xmltv"], default="txt", help="guide output format [xmltv, txt]")
        self.cmdparser.add_option ("-o", dest="filename",  help="store results to file (default to stdout)")
        self.cmdparser.add_option ("-p", dest="parser", choices=["teleman", "gazeta"], default="teleman", help="channel parser source from: [teleman, gazeta] defaults to [teleman]")
    ...
    def SetDefaults(self):                    
        self.default_parser = 'teleman'
        self.parser['teleman'] = TelemanParser.TelemanParser(self.options, self.options.debug_http)
        self.parser['gazeta'] = GazetaParser.GazetaParser(self.options, self.options.debug_http)
    ...
    def ParseCommandLine(self, argv):
        ...
        # ustawienie domyslnego parsera
        self.default_parser = self.options.parser
    ...
    def ProvideExec(self):
        ...
        """
        return self.parser, self.default_parser, self.output        
